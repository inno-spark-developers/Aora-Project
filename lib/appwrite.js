...
function detectAndResolveCycle(obj, seen = new Set()) {
  if (seen.has(obj)) {
    // Cycle detected, replace reference with a surrogate
    return { __cyclicReference__: true };
  }
  seen.add(obj);
  for (const key in obj) {
    const value = obj[key];
    if (typeof value === "object") {
      const resolvedValue = detectAndResolveCycle(value, seen);
      if (resolvedValue.__cyclicReference__) {
        // Replace reference with a surrogate
        obj[key] = { __cyclicReference__: true };
      }
    }
  }
  seen.delete(obj);
...
export const createUser = async (email, password, username) => {
  try {
    ...
    return detectAndResolveCycle(newUser);
  } catch (error) {
    console.log(error);
...
export const signIn = async (email, password) => {
  try {
    ...
    return detectAndResolveCycle(session);
  } catch (error) {
    throw new Error(error);
  }
};
